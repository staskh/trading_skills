...
async def get_expiries(symbol: str, port: int = 7496) -> dict:
    ...
    ib = IB()
    try:
        await ib.connectAsync(host="127.0.0.1", port=port, clientId=20)
    except Exception as e:
        return {
            "success": False,
            "error": f"Could not connect to IB on port {port}: {e}",
        }
...
async def get_option_chain(symbol: str, expiry: str, port: int = 7496) -> dict:
    ...
    ib = IB()
    try:
        await ib.connectAsync(host="127.0.0.1", port=port, clientId=21)
    except Exception as e:
        return {
            "success": False,
            "error": f"Could not connect to IB on port {port}: {e}",
